    ...
    public void repay(Order order) {
        ...
        try {
            ...
            if ("S".equals(returnData.get("resp_code"))
                ...
                //异常不得做为订单状态。
                logger.error("自动扣款，调用宝付协议支付异常，还款订单号={}, rseponse", orderSeriesId, postString);
            }

        } catch (Exception e) {
            logger.error("自动扣款异常，orderId={}", order.getId());
        }
    }
}
