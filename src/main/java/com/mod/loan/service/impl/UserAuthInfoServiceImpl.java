        ...
        if (list.size() > 0) {
            list.stream().forEach(userAuthInfo -> {
                ThreadPoolUtils.executor.execute(() -> {
                    ...
                    OrderUser querOrderUser = new OrderUser();
                    querOrderUser.setOrderNo(userAuthInfo.getOrderNo());
                    OrderUser orderUser = orderUserMapper.selectOne(querOrderUser);
                    if (orderUser == null) {
                        return;
                    }
