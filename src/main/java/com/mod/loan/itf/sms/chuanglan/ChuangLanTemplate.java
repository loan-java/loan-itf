package com.mod.loan.itf.sms.chuanglan;

public enum ChuangLanTemplate {
	T1001("1001","1","【%s】您的验证码为%s，请于%s内正确输入，如非本人操作，请忽略此短信。"),//注册
	T1002("1002","2","【%s】您的验证码为%s，请于%s内正确输入，如非本人操作，请忽略此短信。"),//忘记密码

	T2004("2004","3","【%s】尊敬的用户：您的借款订单已成功打款%s元，请留意款项到账情况。回T退订"),//分期打款
	...
	private ChuangLanTemplate(String key, String templateId, String content) {
		this.key = key;
		this.templateId = templateId;
		this.content = content;
	}

	public static ChuangLanTemplate getTemplate(String key) {
        for (ChuangLanTemplate enumYunpianApikey : ChuangLanTemplate.values()) {
